...
void loopSumo(){
  MoveForwardUntilQTIs(100); 
  if (! digitalRead(leftFrontPhoto) | !digitalRead(rightFrontPhoto)) {
    MoveForward(-2);
    TurnRight(100);
  } else if (!digitalRead(leftBackPhoto)| !digitalRead(rightBackPhoto)) {
    MoveForward(2);
  }
...
long debugQTI() {
  ...
  Serial.print("TestQTI FrontLeft: ");
  Serial.println(RCTime(leftFrontPhoto));
  Serial.print("TestPhoto FrontRight: ");
  Serial.println(RCTime(rightFrontPhoto));
  Serial.print("TestPhoto RearLeft: ");
  Serial.println(RCTime(leftBackPhoto));
  Serial.print("TestPhoto RearRight: ");
  Serial.println(RCTime(rightBackPhoto));
}
void debugWhiskers() {
...
bool TestQTIs() {
  return (!digitalRead(leftFrontPhoto) | !  digitalRead(rightFrontPhoto));
}
